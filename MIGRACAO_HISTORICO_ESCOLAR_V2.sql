-- ========================================================================
-- SCRIPT DE MIGRAÇÃO V2: Reestruturação do Histórico Escolar
-- ========================================================================
-- Descrição: Migra a estrutura atual para o novo modelo normalizado
-- Versão: 2.0 (Corrigido problema de collation)
...
-- ========================================================================
-- IMPORTANTE: Usando utf8mb4_0900_ai_ci para compatibilidade
...
-- Atualizar ordem cronológica dos períodos usando variáveis de sessão
SET @row_number = 0;
...
INNER JOIN (
  SELECT 
    id,
    id_historico,
    @row_number := IF(@current_historico = id_historico, @row_number + 1, 1) AS nova_ordem,
    @current_historico := id_historico
  FROM `historico_escolar_periodo`
  ORDER BY id_historico, ano_letivo
) AS ranked ON p.id = ranked.id
SET p.ordem = ranked.nova_ordem;
...
ALTER TABLE `historico_escolar_notas` 
CHANGE COLUMN `id_historico` `id_historico_old` INT NULL COMMENT 'Coluna antiga - será removida';

-- Renomear coluna id_disciplina para id_historico_disciplina
ALTER TABLE `historico_escolar_notas` 
CHANGE COLUMN `id_disciplina` `id_historico_disciplina` INT NULL COMMENT 'Referência à disciplina do histórico';

-- Criar tabela temporária para mapear id_historico_old para id_periodo
...
INNER JOIN historico_escolar_periodo np ON np.id_historico = h.id 
  ...
  AND np.estabelecimento = p.estabelecimento;

-- Atualizar id_periodo usando a tabela temporária
UPDATE historico_escolar_notas n
INNER JOIN temp_mapping tm ON tm.id_nota = n.id
...
ADD COLUMN `resultado` ENUM('aprovado', 'reprovado', 'dependencia', 'dispensado') 
  CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci
  DEFAULT 'aprovado' COMMENT 'Resultado na disciplina' AFTER `nota`,
ADD COLUMN `faltas` INT DEFAULT 0 COMMENT 'Número de faltas' AFTER `resultado`;
...
ALTER TABLE `historico_escolar_notas` 
MODIFY COLUMN `id_periodo` INT NOT NULL COMMENT 'Referência ao período letivo';
...
SELECT COUNT(*) AS total_notas_migradas FROM historico_escolar_notas WHERE id_periodo IS NOT NULL;

-- ========================================================================
-- PASSO 8: LIMPAR TABELAS TEMPORÁRIAS
-- ========================================================================

-- Remover tabela temporária de períodos
DROP TABLE IF EXISTS `historico_escolar_periodo_temp`;

-- Remover tabela temporária de mapeamento
...
-- Verificar se há notas órfãs (sem período)
SELECT COUNT(*) AS notas_orfas 
FROM historico_escolar_notas 
WHERE id_periodo IS NULL AND deleted_at IS NULL;

-- ========================================================================
-- PASSO 10: RELATÓRIO DE MIGRAÇÃO
-- ========================================================================

SELECT 
  'MIGRAÇÃO CONCLUÍDA COM SUCESSO!' AS status,
  (SELECT COUNT(*) FROM historico_escolar WHERE deleted_at IS NULL) AS total_historicos,
  (SELECT COUNT(*) FROM historico_escolar_periodo WHERE deleted_at IS NULL) AS total_periodos,
  (SELECT COUNT(*) FROM historico_escolar_notas WHERE deleted_at IS NULL) AS total_notas;
...
-- DROP TABLE historico_escolar_periodo;
-- RENAME TABLE historico_escolar_backup TO historico_escolar;
-- RENAME TABLE historico_escolar_notas_backup TO historico_escolar_notas;
-- ========================================================================
